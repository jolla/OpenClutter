...
**v1.1.25** on `dev` — The Terrain resolution control is gone. Terrain stays a checkbox, on by default. When it is on, export uses Auto (cell size from the draw, filling Hamina’s ~20×20 paste budget). When it is off, that export skips the DEM and Copy terrain. The subtitle and muted badge stay (`dev · v1.1.25`).
...
- Hack on **https://openclutter.netlify.app/dev**. The panel corner shows a small muted build label (`dev · v1.1.25`). The same dev host shows **Include foliage** with an Experimental tag (off unless checked) and **Terrain**, on by default. With Terrain on, export uses Auto (cell size from the draw). There is no resolution control.
...
The page has **Include foliage**, unchecked by default. On the dev host only, a **Terrain** checkbox sits under it, on by default. With Terrain on, export uses Auto (cell size from the draw, filling about 20×20). There is no resolution control. The checkbox is hidden wherever the dev badge is hidden. Tree source (NLCD canopy polygons when Include foliage is checked), OSM, and calibration stay automatic or API-only.
