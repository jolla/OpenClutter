...
Terrain still cannot go in the OpenIntent zip. **Copy terrain** pastes open quads into Planner Plus. Flat ground stays a 2×2 pad. A mild rise uses a 4×3 lattice. Relief under 20 m stays 6×5. A ski hill (DEM relief at least 20 m) uses **Auto**: the densest mesh that still fits, aiming at about 1 m cells and stepping coarser only when that grid will not fit beside the zip. A small hill stays near 1 m. A large hill can pass 20×20 when the response can hold it, and the paste is not omitted just because 1 m does not fit. The 3DEP sample count scales with that mesh and stays at most 2500. **Default** is about 80 m quads, at most 12×12 (144 DEM samples) — the same mesh as v1.1.5. **Fine** is about 40 m, at most 16×16 (324 samples). **Finest** is about 25 m, at most 20×20 (576 samples). The page always uses that densest-fit Auto. Default, Fine, and Finest stay available on the API and stay inside 20×20 quads. Quads stay Jerry’s open-quad schema: four corners, ring not closed, sloped floors with one z on the low edge and a higher z on the opposite edge. The ring is counterclockwise in clipboard meters on a north, south, east, or west grade. The grade axis is the larger rise/run, not the larger raw `|Δz|`. `slabOnly` stays false.
...
The page has **Include foliage**, unchecked by default. On the dev host only, a **Terrain** checkbox sits under it, on by default. With Terrain on, export uses the densest mesh that still fits. There is no resolution control. The checkbox is hidden wherever the dev badge is hidden. Tree source (NLCD canopy polygons when Include foliage is checked), OSM, and calibration stay automatic or API-only.
