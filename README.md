...
**v1.1.6** on `dev` — dev-only terrain resolution slider (Default ~80 m, Fine ~40 m, Finest ~25 m) for ski-hill paste tests. CHM canopy crowns and solid (not slab-only) terrain floors stay. The page title and the export zip (`openclutter_version`) both read this from `package.json`.
...
- Hack on **https://openclutter.netlify.app/dev**. The title row shows a **dev** badge and the build version (`dev · v1.1.6`). The same dev host shows **Terrain resolution** (Default / Fine / Finest) next to Include foliage.
...
The page has **Include foliage**, unchecked by default. On the dev host only, **Terrain resolution** sits next to it: a three-stop slider, Default / Fine / Finest, starting on Default. It is hidden wherever the dev badge is hidden. Tree source (NLCD canopy polygons when Include foliage is checked), OSM, and calibration stay automatic or API-only.
...
- `terrainResolution` — `default` (omit it and the server uses this), `fine`, or `finest`. On a ski hill (DEM relief at least 20 m) those are ~80 m / max 12×12, ~40 m / max 16×16, and ~25 m / max 20×20. 3DEP `sampleCount` is 144, 324, or 576 so the finer paste is not interpolated from the default 144-point grid. Relief under 20 m keeps the 2×2, 4×3, or 6×5 ladder. Any other value is Default. The dev page sends this from the Terrain resolution slider. The paste stays at most 20×20 quads.
