...
`hamina-clipboard.json` still carries **Foliage / Tree Trunk** names and exact measured heights. Import OpenIntent for the map, buildings, and tree rings. Do not put a material name outside Hamina’s outdoor Building - One/Two/Five/Ten Floor set into `area_materials` — that empties every attenuation area. Tree rings reuse the gold object for their height bucket.
