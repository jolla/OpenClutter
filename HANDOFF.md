...
| Objects | OpenIntent buildings only (Building - One/Two/Five/Ten Floor) | trees + exact heights on `hamina-clipboard.json` |
...
`hamina-clipboard.json` carries **trees** (Foliage / Tree Trunk) and exact measured building heights. Import OpenIntent for map + buildings; paste the clipboard for foliage. Do not invent OpenIntent material names outside Hamina’s outdoor Building - One/Two/Five/Ten Floor set.
...
OpenIntent outdoor catalog (Jerry’s gold Hamina export): Building - One / Two / Five / Ten Floor only. Clipboard may still use Foliage - Heavy/Light, Tree Trunk, Hotel podium, and measured `bldg-m-*` / `foliage-m-*` types. Schema: OpenIntent 2.0.1 `attenuation_area` = `{ area: { coordinates: [pixel, meter, foot, …] }, area_material }` where each vertex is three consecutive `coordinate_xyz` entries and `area_material` is the material object (Hamina-native keys, no `itu_material_type`). Coords must be ≥ 0 (schema minimum). Invalid / OSM rings historically caused Hamina to drop **all** areas — emit fewer, clipped, closed polygons only.
...
2. Measured heights are emitted. Conflation lives in `netlify/lib/conflate.js`. Same roof: exterior centroid inside a kept ring, or within 11 m of that ring’s centroid. Geometry replace: single exterior, more vertices and area ratio 0.65–1.5, or the kept ring is a stub inside a fuller outline (area ratio 1.35–2.4). Height rank: Overture explicit, then Microsoft Global ML, then FEMA `HEIGHT`, then Overture floors × 3 m, then the nearest measured neighbor within 120 m, then stock bins. Area ratio outside 0.4–2.5 does not copy height onto a different-sized ring. OpenIntent `area_materials` are only Hamina’s outdoor Building - One/Two/Five/Ten Floor names (gold export match). Each `attenuation_area.area_material` is a full copy of that catalog entry with Hamina-native keys only (`name`, `rf_properties`, `top_height`, `display_color` — no `itu_material_type`, no `bottom_height`). Each ring vertex is emitted as consecutive `pixels`, `meters`, `feet` `coordinate_xyz` entries. Pixel aspect equals meter aspect after the Esri snap (isotropic mpu). `reference_markers` is empty. Trees / Foliage / Tree Trunk / Hotel podium are **clipboard-only** — those names emptied every OI attenuation_area for Jerry. Exact metres are `hamina-clipboard.json` zone types (`bldg-m-*`, `foliage-m-*`). `stats.compatibilityMode` is `stock-openintent`. The zip emits at most 982 building areas. Clipboard zones are clipped to the JPEG meter frame (NE = 0,0, SW = −widthM,−lengthM). Tree placement stays NLCD. Meta/WRI CHM v2 (`netlify/lib/canopy-height.js`) sets foliage `top_height` from a zoom-10 COG window resampled to at most 180 px on a side; values at or below 2 m or at or above 80 m are ignored and the NLCD-informed height remains. Parking medians stay a separate imagery pass.
