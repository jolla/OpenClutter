...
Stock Hamina type names (do not invent): Foliage - Heavy/Light, Tree Trunk, Building - One/Five Floor, Hotel podium. Heights and `attenuation_per_m` match the clipboard zone types. Schema: OpenIntent 2.0.1 `attenuation_area` = `{ area: { coordinates: [{coordinate_xyz:{x,y,unit:"pixels"}}] }, area_material }` where `area_material` is the material object, not a catalog name. Coords must be ≥ 0 (schema minimum). Invalid / OSM rings historically caused Hamina to drop **all** areas — emit fewer, clipped, closed polygons only.
