...
1. Esri MSBFP2 (`dataLastEditDate` 2022-04-13) is not the full Microsoft set. The Oak Creek repro export (JPEG 877×1046, extent in `test/fixtures/oak-creek-commercial/bbox.json`) fetched **49** MSBFP2 footprints and kept **48** (one tiny). Hamina imported all **558** emitted areas. The large white retail roof is not in those 49. Global ML Building Footprints for quadkey `030222210` clips to **68** polygons on that extent; merged with MSBFP2 that is **74** kept, including that roof. Index: `netlify/lib/ms-buildings-index.json`. Refresh when Microsoft moves `dataset-links.csv` (currently `https://bfppub.blob.core.windows.net/$web/2026-07-24/dataset-links.csv`, UnitedStates rows only). A failed or slow (>8s) global download falls back to MSBFP2 alone. OSM building ways are still not fetched.
